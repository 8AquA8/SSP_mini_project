...
def vqe_ground_state_properties(nq, device, n_layers, shots, weights, j=1, h=-1):
    qc = vqe_PQC(device, nq, n_layers, shots)
...
def train(device, shots, site_numbers, n_layers, h_values, j):
    ...
    for n_sites in site_numbers:
        weights = torch.zeros(n_sites*n_layers-1, requires_grad=True)
        qc = vqe_PQC(device, n_sites, n_layers, shots)
        best_h_weights = []
        ...
        for h in h_values:
            ...
            save_path = f"best_weights_n{n_sites}_h{h:.4f}.pt"
...
plt.legend()
plt.grid(True)
plt.savefig('ground_state_energy.png')  # Save the energy plot
...
plt.legend()
plt.grid(True)
plt.savefig('ground_state_magnetization.png')  # Save the magnetization plot
plt.close()  # Close the plot